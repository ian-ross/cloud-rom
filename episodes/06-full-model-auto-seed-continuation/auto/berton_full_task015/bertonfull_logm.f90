...
SUBROUTINE FUNC(NDIM,U,ICP,PAR,IJAC,F,DFDU,DFDP)
  ...
  DOUBLE PRECISION :: uu(4),ff(4),up(4),um(4),fp(4),fm(4),pp(95),pm(95),h,steps(4),pstep
  INTEGER :: j,k
  uu(1:4)=U(1:4); CALL rhs_full(uu,PAR,ff); F(1:4)=ff(1:4)
  IF (IJAC .NE. 0) THEN
    DFDU(1:NDIM,1:NDIM)=0D0
    DFDP(1:NDIM,1:95)=0D0
    ! Analytic kinematic row: dz/dt = w. Remaining coupled microphysics
    ! derivatives are centered finite differences in the reformulated coordinates.
    DFDU(1,3)=1D0
    steps=(/1D0,1D-5,1D-5,1D-4/)
    DO j=1,4
    ...
    END DO
    DFDU(1,1)=0D0; DFDU(1,2)=0D0; DFDU(1,3)=1D0; DFDU(1,4)=0D0
    DO j=1,4
      k=ICP(j)
      IF (k .GT. 0 .AND. k .LE. 95) THEN
        pstep=MAX(ABS(PAR(k))*1D-5,1D-6)
        pp(1:95)=PAR(1:95); pm(1:95)=PAR(1:95)
        pp(k)=PAR(k)+pstep; pm(k)=PAR(k)-pstep
        CALL rhs_full(uu,pp,fp); CALL rhs_full(uu,pm,fm)
        DFDP(1:4,k)=(fp(1:4)-fm(1:4))/(pp(k)-pm(k))
      END IF
    END DO
  END IF
END SUBROUTINE FUNC
...
SUBROUTINE STPNT(NDIM,U,PAR,T)
  ...
  PAR(39)=0D0
  PAR(43)=0D0
  U(1)=9.618027532260936D3
  U(2)=1.9098623386953226D0
  U(3)=-2.9084536311763646D-21
  ! U(4)=log(m/kg) for positivity and better mass-coordinate conditioning.
  U(4)=LOG(1.0802293920592054D-9)
