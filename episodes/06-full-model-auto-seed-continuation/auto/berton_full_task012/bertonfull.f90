...
MODULE berton_full_model
  ...
  DOUBLE PRECISION, PARAMETER :: mass_scale = 1.0D-9
...
CONTAINS
  ...
  SUBROUTINE rhs_full(uvec,par,f)
    ...
    TYPE(Local) :: L
    DOUBLE PRECISION :: fc
    CALL eval_local(uvec(1),uvec(2),uvec(3),mass_scale*uvec(4),par,L)
    fc=par(43)*par(47)*COS(par(42))
    f(1)=uvec(3)
    f(2)=-L%k*(uvec(2)-L%U_a)-2D0*fc*uvec(3)
    f(3)=L%vertical_force
    f(4)=L%m_dot/mass_scale
  ...
  SUBROUTINE set_pvls(uvec,par)
    ...
    CALL eval_local(uvec(1),uvec(2),uvec(3),mass_scale*uvec(4),par,L)
    dz=1D0; base=uvec; base(1)=uvec(1)+dz; CALL eval_local(base(1),base(2),base(3),mass_scale*base(4),par,Lp)
    base=uvec; base(1)=uvec(1)-dz; CALL eval_local(base(1),base(2),base(3),mass_scale*base(4),par,Lm)
    ...
    dmuse=MAX(ABS(uvec(4))*1D-4,1D-8); base=uvec; base(4)=uvec(4)+dmuse; CALL rhs_full(base,par,Fp)
    base=uvec; base(4)=MAX(uvec(4)-dmuse,1D-12); CALL rhs_full(base,par,Fm)
...
SUBROUTINE FUNC(NDIM,U,ICP,PAR,IJAC,F,DFDU,DFDP)
  ...
  IF (IJAC .NE. 0) THEN
    ...
    steps=(/1D0,1D-5,1D-5,MAX(ABS(uu(4))*1D-4,1D-8)/)
    DO j=1,4
...
SUBROUTINE STPNT(NDIM,U,PAR,T)
  ...
  ! U(4) is scaled as m / 1e-9 kg for AUTO conditioning.
  U(4)=1.0802293920592054D0
