...
!
! U(1)=z [m], U(2)=u [m/s], U(3)=w [m/s], U(4)=log(m/kg).
...
CONTAINS
  ...
  DOUBLE PRECISION FUNCTION softplus_stable(x)
    DOUBLE PRECISION, INTENT(IN) :: x
    IF (x > 40D0) THEN
      softplus_stable = x
    ELSEIF (x < -40D0) THEN
      softplus_stable = EXP(x)
    ELSE
      softplus_stable = LOG(1D0+EXP(x))
    END IF
  END FUNCTION softplus_stable

  DOUBLE PRECISION FUNCTION smooth_clamp01(x,eps)
    DOUBLE PRECISION, INTENT(IN) :: x,eps
    ! Smooth approximation to min(max(x,0),1):
    ! eps*[softplus(x/eps)-softplus((x-1)/eps)].
    ! eps is dimensionless; eps=50/Delta_z_W corresponds to a 50 m
    ! smoothing width for the Berton updraft ramp endpoints.
    smooth_clamp01 = eps*(softplus_stable(x/eps)-softplus_stable((x-1D0)/eps))
  END FUNCTION smooth_clamp01

  DOUBLE PRECISION FUNCTION updraft(z,par)
    DOUBLE PRECISION, INTENT(IN) :: z,par(*)
    DOUBLE PRECISION :: zW1,x,eps_m,eps_x
    zW1 = par(1)-par(7)
    eps_m = par(56)
    IF (eps_m <= 0D0) eps_m = 50D0
    eps_x = eps_m/par(7)
    x = (z-zW1)/par(7)
    updraft = par(2)*smooth_clamp01(x,eps_x)
  ...
  SUBROUTINE rhs_full(uvec,par,f)
    ...
    TYPE(Local) :: L
    DOUBLE PRECISION :: fc
    CALL eval_local(uvec(1),uvec(2),uvec(3),EXP(uvec(4)),par,L)
    fc=par(43)*par(47)*COS(par(42))
    f(1)=uvec(3)
    f(2)=-L%k*(uvec(2)-L%U_a)-2D0*fc*uvec(3)
    f(3)=L%vertical_force
    f(4)=L%m_dot/MAX(EXP(uvec(4)),1D-300)
  ...
  SUBROUTINE set_pvls(uvec,par)
    ...
    CALL eval_local(uvec(1),uvec(2),uvec(3),EXP(uvec(4)),par,L)
    dz=1D0; base=uvec; base(1)=uvec(1)+dz; CALL eval_local(base(1),base(2),base(3),EXP(base(4)),par,Lp)
    base=uvec; base(1)=uvec(1)-dz; CALL eval_local(base(1),base(2),base(3),EXP(base(4)),par,Lm)
    ...
    dmuse=1D-4; base=uvec; base(4)=uvec(4)+dmuse; CALL rhs_full(base,par,Fp)
    base=uvec; base(4)=uvec(4)-dmuse; CALL rhs_full(base,par,Fm)
...
!----------------------------------------------------------------------
! TASK-023 smoothed-z_W0 P-scaled restricted/scaled 3D equilibrium continuation.
!
! AUTO states are scaled local-balance variables recommended by TASK-018:
!   U(1)=Z=(z-z_seed)/100 m
!   U(2)=Us=(u-u_seed)/(1 m/s)
!   U(3)=P=M/10 where M=log(m/m_seed)
! with w fixed to zero.  The physical inverse map is
!   z=z_seed+100*Z, u=u_seed+Us, w=0, m=m_seed*exp(10*P).
! Residuals are row-scaled (du/dt, dw/dt, dlog(m)/dt) balances.
!----------------------------------------------------------------------
SUBROUTINE scaled_to_full(U, UU)
  USE berton_full_model
  IMPLICIT NONE
  DOUBLE PRECISION, INTENT(IN) :: U(3)
  DOUBLE PRECISION, INTENT(OUT) :: UU(4)
  DOUBLE PRECISION, PARAMETER :: z_seed=9.61802753226093591D+03, u_seed=1.90986233869532240D+00, m_seed=1.08022939205920521D-09
  UU(1)=z_seed + 100D0*U(1)
  UU(2)=u_seed + U(2)
  UU(3)=0D0
  UU(4)=LOG(m_seed) + 10D0*U(3)
END SUBROUTINE scaled_to_full

SUBROUTINE restricted_scaled_rhs(U, PAR, F)
  USE berton_full_model
  IMPLICIT NONE
  DOUBLE PRECISION, INTENT(IN) :: U(3), PAR(*)
  DOUBLE PRECISION, INTENT(OUT) :: F(3)
  DOUBLE PRECISION :: UU(4), FF(4)
  DOUBLE PRECISION, PARAMETER :: s_du=1.63422983821310730D+01, s_dw=6.06158311374814662D+00, s_dlogm=1.28877212130016035D-06
  CALL scaled_to_full(U, UU)
  CALL rhs_full(UU, PAR, FF)
  F(1)=FF(2)/s_du
  F(2)=FF(3)/s_dw
  F(3)=FF(4)/s_dlogm
END SUBROUTINE restricted_scaled_rhs
...
SUBROUTINE FUNC(NDIM,U,ICP,PAR,IJAC,F,DFDU,DFDP)
  ...
  DOUBLE PRECISION :: PAREFF(98)
  ! TASK-023 intentionally uses JAC=0 first so AUTO finite-differences
  ! in the arclength-scaled P=M/10 mass coordinate.  If a supplied
  ! Jacobian is reintroduced, DFDU(:,3) must include the dM/dP=10 factor.
  PAREFF(1:98)=PAR(1:98)
  PAREFF(1)=9000D0 + 1000D0*PAR(54)
  PAREFF(55)=PAREFF(1)
  CALL restricted_scaled_rhs(U(1:3),PAREFF,F(1:3))
END SUBROUTINE FUNC
...
SUBROUTINE PVLS(NDIM,U,PAR)
  ...
  DOUBLE PRECISION, INTENT(INOUT) :: PAR(*)
  ! Deliberately empty for the z_W0 gate: parser reconstructs physical
  ! diagnostics from Z, U, and P=M/10 without PVLS bookkeeping in ICP.
END SUBROUTINE PVLS
