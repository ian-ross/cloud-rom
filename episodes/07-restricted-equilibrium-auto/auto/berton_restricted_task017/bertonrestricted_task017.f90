...
!----------------------------------------------------------------------
! TASK-017 restricted/scaled 3D equilibrium gate.
!
...
!   z=z_seed+100*Z, u=u_seed+Us, w=0, m=m_seed*exp(M).
...
SUBROUTINE FUNC(NDIM,U,ICP,PAR,IJAC,F,DFDU,DFDP)
  ...
  DOUBLE PRECISION :: up(3),um(3),fp(3),fm(3),pp(98),pm(98),h,steps(3),pstep
  INTEGER :: j,k
  CALL restricted_scaled_rhs(U(1:3),PAR,F(1:3))
  IF (IJAC .NE. 0) THEN
    DFDU(1:NDIM,1:NDIM)=0D0
    DFDP(1:NDIM,1:98)=0D0
    steps=(/1D-4,1D-5,1D-4/)
    DO j=1,3
      h=steps(j); up=U(1:3); um=U(1:3); up(j)=up(j)+h; um(j)=um(j)-h
      CALL restricted_scaled_rhs(up,PAR,fp); CALL restricted_scaled_rhs(um,PAR,fm)
      DFDU(1:3,j)=(fp(1:3)-fm(1:3))/(up(j)-um(j))
    END DO
    DO j=1,MIN(4,NDIM)
      k=ICP(j)
      IF (k .GT. 0 .AND. k .LE. 98) THEN
        pstep=MAX(ABS(PAR(k))*1D-5,1D-6)
        pp(1:98)=PAR(1:98); pm(1:98)=PAR(1:98)
        pp(k)=PAR(k)+pstep; pm(k)=PAR(k)-pstep
        CALL restricted_scaled_rhs(U(1:3),pp,fp); CALL restricted_scaled_rhs(U(1:3),pm,fm)
        DFDP(1:3,k)=(fp(1:3)-fm(1:3))/(pp(k)-pm(k))
      END IF
    END DO
  END IF
END SUBROUTINE FUNC
...
SUBROUTINE PVLS(NDIM,U,PAR)
  ...
  CALL scaled_to_full(U(1:3), uu)
  CALL set_pvls(uu,PAR)
  PAR(96)=uu(1)
  PAR(97)=uu(2)
  PAR(98)=EXP(uu(4))
END SUBROUTINE PVLS
