...
!----------------------------------------------------------------------
! TASK-020 H-scaled restricted/scaled 3D H_a3 continuation.
!
! AUTO states reuse the TASK-019 W_a0-gated local-balance variables:
!   U(1)=ZH=(z-z_seed)/1000 m
!   U(2)=UH=(u-u_seed)/(5 m/s)
!   U(3)=P=M/10 where M=log(m/m_seed)
! with w fixed to zero.  The physical inverse map is
!   z=z_seed+1000*ZH, u=u_seed+5*UH, w=0, m=m_seed*exp(10*P).
! H_a3 is continued through an arclength-scaled active control
!   PAR(54)=q_H=(H_a3-0.61)/0.001, so H_a3=0.61+0.001*q_H.
...
SUBROUTINE FUNC(NDIM,U,ICP,PAR,IJAC,F,DFDU,DFDP)
  ...
  ! TASK-019 intentionally uses JAC=0 first so AUTO finite-differences
