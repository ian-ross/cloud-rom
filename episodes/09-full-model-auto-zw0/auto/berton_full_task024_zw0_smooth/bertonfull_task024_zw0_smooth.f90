...
!
! AUTO variables are scaled around the TASK-011/TASK-012 seed:
! U(1)=Z=(z-z_seed)/1000, U(2)=U_h=(u-u_seed)/5,
! U(3)=W=w, U(4)=P=(log(m/m_seed))/10.
! PAR(96)=q_z=(z_W0-9000 m)/1000 m; PAR(99)=50 m smoothing width.
...
CONTAINS
  ...
  SUBROUTINE scaled_to_physical(us,par,uphys,workpar)
    DOUBLE PRECISION, INTENT(IN) :: us(4), par(*)
    DOUBLE PRECISION, INTENT(OUT) :: uphys(4), workpar(110)
    workpar(1:95)=par(1:95)
    workpar(110)=par(96)
    workpar(1)=9000D0 + 1000D0*par(96)
    workpar(55)=workpar(1)
    workpar(99)=par(99)
    uphys(1)=9.618027532260936D3 + 1000D0*us(1)
    uphys(2)=1.9098623386953226D0 + 5D0*us(2)
    uphys(3)=us(3)
    uphys(4)=LOG(1.0802293920592054D-9) + 10D0*us(4)
  END SUBROUTINE scaled_to_physical

  SUBROUTINE rhs_scaled(us,par,fscaled)
    DOUBLE PRECISION, INTENT(IN) :: us(4), par(*)
    DOUBLE PRECISION, INTENT(OUT) :: fscaled(4)
    DOUBLE PRECISION :: uphys(4), workpar(110), fphys(4)
    CALL scaled_to_physical(us,par,uphys,workpar)
    CALL rhs_full(uphys,workpar,fphys)
    ! Row scaling keeps the kinematic, acceleration, and log-growth residuals
    ! order-one near the seed without changing the equilibrium roots.
    fscaled(1)=fphys(1)/1D0
    fscaled(2)=fphys(2)/1D-3
    fscaled(3)=fphys(3)/1D-3
    fscaled(4)=fphys(4)/1D-5
  END SUBROUTINE rhs_scaled
...
SUBROUTINE STPNT(NDIM,U,PAR,T)
  ...
  ! TASK-011 damped/equilibrium-like seed at canonical oscillatory Case 0:
  ! z_W0=9000 m, W_a0=0.6 m/s, z_W0=0.61, include_coriolis=0.
  PAR(1)=9000D0
  PAR(2)=0.6D0
  PAR(4)=0.61D0
  PAR(39)=0D0
...
SUBROUTINE PVLS(NDIM,U,PAR)
  ...
  DOUBLE PRECISION, INTENT(INOUT) :: PAR(*)
  DOUBLE PRECISION :: us(4), uphys(4), workpar(110), fscaled(4)
  us(1:4)=U(1:4)
  CALL scaled_to_physical(us,PAR,uphys,workpar)
  PAR(1)=workpar(1)
  PAR(54)=PAR(96)
  ...
  PAR(59)=EXP(uphys(4))
  PAR(97)=10D0*us(4)
  CALL rhs_scaled(us,PAR,fscaled)
  PAR(98)=SQRT(SUM(fscaled(1:4)**2))
  CALL set_pvls(uphys,workpar)
  PAR(60:95)=workpar(60:95)
END SUBROUTINE PVLS
