...
This note records the reproducible build/run and validation commands for the full four-state Berton AUTO problem in `auto/berton_full/bertonfull.f90`.
...
This compiles `auto/berton_full/bertonfull.f90` with the installed AUTO-07p environment and saves:

- `auto/berton_full/b.bertfull-zW0`
- `auto/berton_full/s.bertfull-zW0`
- `auto/berton_full/d.bertfull-zW0`

The continuation constants are in `auto/berton_full/c.bertonfull`; the primary continuation parameter is `PAR(1)=z_W0`, with user output at 10000, 9500, 9000, and 8500 m.
...
```bash
uv run python scripts/berton_full_auto_task009_validate.py --run-auto
```
